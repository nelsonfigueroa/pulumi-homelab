...
backend_container = docker.Container(
    ...
    opts=pulumi.ResourceOptions(depends_on=[mongo_container]), # use depends_on to ensure other containers start up first.
)
...
ip = frontend_container.ports[0].ip
port = frontend_container.ports[0].external

# Combine IP address and port
address = pulumi.Output.all(ip, port).apply(lambda vals: f"http://{vals[0]}:{vals[1]}")

# Export address
pulumi.export('address', address)
